...
void _printBytes(uint8_t *bytes, int length) {
  printf("RAND SEED [ ");
...
static int g_rng_function2(uint8_t *dest, unsigned size) {
  //for (int i = 0; i < size; i++) {
  //  dest[i] = i;
  //}

  int result = g_rng_function(dest, size);
  _printBytes(dest, size);
  return result;
}
...
int _uECC_sign(const uint8_t *private_key,
              const uint8_t *message_hash,
              unsigned hash_size,
              uint8_t *signature,
              uECC_Curve curve) {
